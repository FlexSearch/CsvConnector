...
Target "BuildApp" <| fun _ -> 
    AssemblyInfo "CsvConnector" "FlexSearch CSV Connector"
    MSBuildRelease buildDir "Build" [ @"src\CsvConnector.sln" ] |> Log "BuildApp-Output: "
    // Copy the files from build to build-test necessary for Testing
    FileHelper.CopyRecursive buildDir testDir true |> ignore

Target "Test" (fun _ -> 
    !! (testDir @@ "CsvConnector.Tests.dll") 
    |> (fun includes ->
...
Target "Zip" <| fun _ -> 
    !! (buildDir + "/**/CsvConnector.dll")
    ++ (buildDir + "/csvconnector/**/*.*")
    |> Zip buildDir (deployDir <!!> "CsvConnector." + version + ".zip")
...
Target "BuildPortal" <| fun _ ->
    ...
    ensureDirectory (portalBuildDir <!!> "src/apps/csvconnector")
    FileHelper.CopyRecursive portalDir (portalBuildDir <!!> "src/apps/csvconnector") true |> ignore
...
Target "DeployPortal" <| fun _ ->
    ensureDirectory (buildDir <!!> "csvconnector")
    let baseSource = (portalBuildDir <!!> "src/apps/csvconnector/dist")
    !! (baseSource + "/**/*.*") 
    |> Seq.iter (FileHelper.CopyFileWithSubfolder baseSource (buildDir <!!> "csvconnector"))
